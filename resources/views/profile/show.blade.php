<x-app-layout>
    ...
    <div x-data="{ 
        ...
        // Tato funkce stáhne data a synchronizuje obě tabulky recenzí
        ...
        // Tato funkce provede smazání nebo změnu role a IHNED překreslí tabulku
        async apiAction(url, method = 'DELETE', typeToRefresh, id = null) {
            ...
            try {
                ...
                if (response.ok) {
                    // Pokud mažeme recenzi, vyhodíme ji ze seznamu 'myReviews' okamžitě
                    if (typeToRefresh === 'reviews' && method === 'DELETE' && id) {
                        this.myReviews = this.myReviews.filter(r => r.id !== id);
                    }
                    // Aktualizujeme data v tabulce ze serveru
                    await this.fetchData(typeToRefresh);
                } else { 
        ...
        async savePub() {
            ...
            try {
                ...
                if (response.ok) { 
                    this.isPubModalOpen = false; // Zavře modál
                    await this.fetchData('pubs'); // Ihned aktualizuje seznam hospod v tabulce
                } 
                    ...
                    <div class="bg-gray-900 border border-gray-700 p-4 rounded flex justify-between items-start transition hover:bg-gray-800 hover:border-gray-500 group">
                        ...
                        <div class="flex flex-col items-end gap-2 ml-4">
                            ...
                            <div class="flex items-center gap-2">
                                <button @click="openEditModal(review)" class="text-gray-400 hover:text-white bg-gray-800 border border-gray-600 hover:bg-gray-700 p-1 rounded transition" title="Upravit"><svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-4 h-4"><path stroke-linecap="round" stroke-linejoin="round" d="m16.862 4.487 1.687-1.688a1.875 1.875 0 1 1 2.652 2.652L10.582 16.07a4.5 4.5 0 0 1-1.897 1.13L6 18l.8-2.685a4.5 4.5 0 0 1 1.13-1.897l8.932-8.931Zm0 0L19.5 7.125M18 14v4.75A2.25 2.25 0 0 1 15.75 21H5.25A2.25 2.25 0 0 1 3 18.75V8.25A2.25 2.25 0 0 1 5.25 6H10" /></svg></button>
                                <button @click="apiAction('/reviews/' + review.id, 'DELETE', 'reviews')" 
                                        class="text-red-500 hover:text-red-400 bg-gray-800 border border-gray-600 hover:bg-gray-700 p-1 rounded transition" 
                                        title="Smazat">
            ...
            <div class="space-y-4">
                <template x-for="review in adminReviews" :key="review.id">
                    <div class="bg-gray-900 border border-gray-700 p-4 rounded flex justify-between items-start transition hover:bg-gray-800 hover:border-gray-500"
                        ...
                        <button @click="apiAction('/reviews/' + review.id, 'DELETE', 'reviews')" 
                                class="text-red-500 hover:text-red-700 font-bold text-sm border border-red-500 px-3 py-1 rounded hover:bg-red-500 hover:text-black transition ml-4">
                            SMAZAT
                        </button>
                    </div>
                </template>
                                        ...
                                        <template x-if="user.id !== {{ auth()->id() }}">
                                            <div class="flex gap-2">
                                                ...
                                                <button @click="apiAction('/admin/users/' + user.id, 'DELETE', 'users')" 
                                                        class="bg-gray-700 text-red-500 hover:bg-red-500 hover:text-white p-2 rounded transition" title="Smazat uživatele">
                ...
                <template x-for="pub in adminPubs" :key="pub.id">
                    <div class="bg-gray-900 border border-gray-700 p-4 rounded flex justify-between items-center transition hover:bg-gray-800 hover:border-gray-500"
                        ...
                        <div class="flex items-center space-x-3">
                            ...
                            <button @click="apiAction('/admin/pubs/' + pub.id, 'DELETE', 'pubs')" 
                                    class="text-red-500 hover:text-white border border-red-500 hover:bg-red-600 px-3 py-1 rounded transition">
                                SMAZAT
                            </button>
                        </div>
                    </div>
        ...
        <!-- MODÁLY (RECENZE, USER, PUB) -->
        <!-- Modál pro úpravu recenze -->
        ...
        <!-- Modál pro přidání/úpravu podniku -->
